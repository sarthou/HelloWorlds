#ifndef HWS_COMMON_URDF_H
#define HWS_COMMON_URDF_H
...
namespace hws {
  ...
  public:
    ...
    virtual void addLink(const std::string& parent,
                         ...
                         const std::vector<hws::Shape>& visual_shapes) = 0;
    ...
    std::string name_;
    hws::Actor* root_actor_;
    std::unordered_map<std::string, hws::Actor*> links_;
    std::unordered_map<size_t, hws::Actor*> id_links_;
    std::unordered_map<std::string, hws::Joint*> joints_;
    std::unordered_map<std::string, hws::Material> materials_;
  };
} // namespace hws

#endif // HWS_COMMON_URDF_H