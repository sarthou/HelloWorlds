...
uniform sampler2D texture_ssao; // blurred SSAO texture
...
{
  // 1. SSAO Lookup
  vec2 screenUV = gl_FragCoord.xy / viewport_size;
  float ssao = texture(texture_ssao, screenUV).r;
  ...
  // Apply SSAO to directional ambient
  totalAmbient += dir_light.ambient.rgb * albedo * ssao;
  ...
  for(int i = 0; i < int(nb_point_lights); i++)
  {
    ...
    vec3 L_vec = point_lights[i].position.xyz - FragPos;
    ...
    // Apply SSAO to point light ambient
    totalAmbient += point_lights[i].ambient.rgb * albedo * attenuation * ssao;
  ...
  // Simple Exposure + ACES Tonemapping
