...
namespace hws {
  ...
  size_t World::createStaticActor(const hws::urdf::Geometry_t& collision_geometry,
                                  const std::vector<hws::urdf::Geometry_t>& visual_geometries,
                                  const std::array<double, 3>& position,
                                  const std::array<double, 4>& rotation)
  {
    hws::Shape collision_shape = convertShape(collision_geometry);
    std::vector<hws::Shape> visual_shapes;
  ...
  size_t World::createActor(const hws::urdf::Geometry_t& collision_geometry,
                            const std::vector<hws::urdf::Geometry_t>& visual_geometries,
                            const std::array<double, 3>& position,
                            const std::array<double, 4>& rotation)
  {
  ...
  size_t World::createVisualActor(const std::vector<hws::urdf::Geometry_t>& visual_geometries,
                                  const std::array<double, 3>& position,
                                  const std::array<double, 4>& rotation)
  {
    std::vector<hws::Shape> visual_shapes;
  ...
  int World::getNumJoints(size_t urdf_id) const
  {
    ...
    if(it == urdfs_.end())
      ...
      return (int)it->second->getNumJoints();
  }
                          ...
                          int link_index)
  {
    ...
    for(const auto& vertex : vertices)
      glm_vertices.emplace_back(toGlmV3(vertex));
                           ...
                           const std::array<double, 3>& orientation)
  {
    ...
    hws::Shape visual_shape = convertShape(link.visual);
    std::vector<hws::Shape> collision_shapes;
    ...
    if(parent.empty())
      urdf->addLink(parent, link_name, toGlmV3(position), toGlmV3(orientation), collision_shapes.front(), {visual_shape});
    else
    {
  ...
  hws::Shape World::createShapeBox(const hws::Color& color, const std::array<float, 3>& half_extents, glm::mat4& transform)
  {
    return hws::ShapeBox{
      half_extents,
      color,
  ...
  hws::Shape World::createShapeCapsule(const hws::Color& color, const float radius, const float height, glm::mat4& transform)
  {
    return hws::ShapeCapsule{
      radius,
      height,
      color,
  ...
  hws::Shape World::createShapeCylinder(const hws::Color& color, const float radius, const float height, glm::mat4& transform)
  {
    return hws::ShapeCylinder{
      radius,
      height,
      color,
      preloaded_cylinder_model_,
  ...
  hws::Shape World::createShapeSphere(const hws::Color& color, const float radius, glm::mat4& transform)
  {
    return hws::ShapeSphere{
      radius,
      color,
      preloaded_sphere_model_,
      transform};
  ...
  hws::Shape World::createShapeFromModel(const hws::Material& material, const std::string& path, const std::array<float, 3>& scale, glm::mat4& transform)
  {
    return hws::ShapeCustomMesh{
      glm::vec3(scale[0], scale[1], scale[2]),
      material,
      hws::ModelManager::get().load(path),
      transform};
  }

  hws::Shape World::convertShape(const urdf::Geometry_t& geometry)
  {
  ...
  hws::Shape World::convertShape(const urdf::Geometry_t& urdf_shape, glm::mat4& transform)
  {
