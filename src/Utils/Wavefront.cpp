#include "hello_worlds/Utils/Wavefront.h"
...
namespace hws::wavefront {
  ...
  std::vector<std::array<double, 3>> getVertexes(const std::string& obj_path)
  {
    ...
    std::regex v_regex(R"(v\s([-|\d|.]+)\s([-|\d|.]+)\s([-|\d|.]+))");
    std::regex l_regex(R"(l\s([\d]+)\s([\d]+))");
    std::smatch match;
  ...
  std::string getMltFile(const std::string& obj_path)
  {
    ...
    if(obj_f)
    {
      ...
      if(mlt_pose != std::string::npos)
      {
        ...
        mlt_path.replace(mlt_path.begin() + (long)last_pose, mlt_path.end(), "/" + mlt_name);
  ...
  std::string getMaterialColor(const std::string& mlt_content, const std::string& material_name)
  {
    ...
    if(material_pose != std::string::npos)
    {
      ...
      std::regex regex(R"(Kd\s([\d|.]+)\s([\d|.]+)\s([\d|.]+))");
      std::smatch match;
      if(std::regex_match(color_str, match, regex))
      {
        int r = (int)(std::stod(match[1].str()) * 255.0);
        int g = (int)(std::stod(match[2].str()) * 255.0);
        int b = (int)(std::stod(match[3].str()) * 255.0);
