...
namespace hws {
  ...
  void MsaaScreen::init(unsigned int width, unsigned int height)
  {
    width_ = (int)width;
    height_ = (int)height;
  ...
  void MsaaScreen::reinit(unsigned int width, unsigned int height)
  {
    if(init_ == false)
      return;
  ...
  void MsaaScreen::initBuffers(unsigned int msaa_samples)
  {
  ...
  void MsaaScreen::bindFrameBuffer() const
  {
    glBindFramebuffer(GL_FRAMEBUFFER, msaa_framebuffer_);
    glViewport(0, 0, width_, height_);
  }

  void MsaaScreen::generateColorTexture() const
  {
  ...
  void MsaaScreen::renderQuad() const
  {
    glBindVertexArray(screen_vao_);
  ...
  std::array<float, 24> MsaaScreen::screen_vertices = {
    // positions // texCoords
    -1.0f, 1.0f, 0.0f, 1.0f,
    -1.0f, -1.0f, 0.0f, 0.0f,
    1.0f, -1.0f, 1.0f, 0.0f,
