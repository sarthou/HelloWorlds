...
namespace hws {
  ...
  void GeometryBuffer::init(unsigned int width, unsigned int height)
  {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, (int)width_, (int)height_, 0, GL_RGBA, GL_FLOAT, nullptr);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, (int)width_, (int)height_, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    ...
    unsigned int attachments[1] = {GL_COLOR_ATTACHMENT0}; // NOLINT
